    ...
    async def test_real_stdio_client(self):
        ...
        async with stdio_client(StdioServerParameters(command=sys.executable,
            ...
            async with ClientSession(read, write) as session:
                ...
                self.assertEqual({t.name for t in tools.tools}, {'tyc_get_access_status', 'tyc_search_companies', 'tyc_get_company'})
                status = await session.call_tool('tyc_get_access_status', {})
                self.assertFalse(status.isError)
                self.assertFalse(status.structuredContent['browser_connected'])
                invalid = await session.call_tool('tyc_get_company', {'company_id': '../evil'})
                self.assertTrue(invalid.isError)
                disconnected = await session.call_tool('tyc_get_company', {'company_id': '123'})
