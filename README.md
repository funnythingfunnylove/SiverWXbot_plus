...
在控制面板「MCP 工具」中添加 Streamable HTTP 服务、测试连接、勾选工具，并为私聊或群聊授权（授权群内所有成员 @ 机器人即可使用）。支持工具调用的 OpenAI 兼容模型可以自动查询工具并根据结果回复微信。功能默认关闭，MCP 配置单独保存，修改后下次对话生效。

内置项目管理系统（HRZH）入口：点击「添加项目管理用户」，为每个微信私聊窗口绑定个人 Key，验证绑定人员后选择该 Key 可用的工具。个人 Key 独立使用，失败或停用不会回退共享 Key。

支持私聊主动提醒：直接说“10 分钟后提醒我开会”“每天 9 点提醒我签到”，到点由机器人主动发送，支持查看和取消，重启后保留。定时项目查询使用本人 Key 的只读工具；机器人和微信需保持运行，时间按北京时间 UTC+08:00。

更新后需安装新的依赖并重启整个 Python 进程。详细接入范围、认证、权限及 Windows 实机测试步骤见 [MCP 使用说明](docs/mcp.md)。
